...
export async function setup(app: Ycs) {
  ...
  const docPaths = docs.map(x => x.docs).reduce((a, b) => {
...
export const SimpleAuthRouter = new Router('/auth/simple').paths(
  {
    path: '/signin',
    methods: ['post'],
    controller: async (ctx: IContext) => {
      try {
        ...
        const token = signToken(auth, { expiresIn: Ycs.instance.config.auth.expiresIn || '1h' });
